...
const HOTSPOT_ICON =
  "data:image/svg+xml;utf8,<svg width='32' height='32' xmlns='http://www.w3.org/2000/svg'><circle cx='16' cy='16' r='14' fill='%23f43f5e' stroke='white' stroke-width='3'/></svg>";
...
const PanoramaViewer = ({
  ...
  setShowInfo,
  setLoading,
  onHotspotClick, // NEW: Add callback for hotspot clicks
}) => {
  ...
  useEffect(() => {
    ...
    loader.load(
      ...
      (texture) => {
        ...
        // Add hotspots from data - safely handle case when hotspots might be undefined
        if (property.hotspots && Array.isArray(property.hotspots)) {
          const hotspotTexture = new THREE.TextureLoader().load(HOTSPOT_ICON);
          property.hotspots.forEach((hotspot, index) => {
        ...
        function onDoubleTap() {  // Remove unused 'e' parameter
          const now = Date.now();
          if (now - lastTap < 300) {
            lon = 0;
            lat = 0;
        ...
        function onClick(event) {
          ...
          // Only do intersection test if we have hotspot objects
          if (hotspotObjects.length > 0) {
            ...
            if (intersects.length > 0) {
              ...
              // NEW: Pass hotspot data to the parent component
              if (onHotspotClick) {
  ...
  }, [property, gyroEnabled, setLonState, setShowInfo, setLoading, mountRef, onHotspotClick]);
